...
The experimental in-Fusion path is implemented in `steve.live_update` and owned by `STEVEUpdater`, not by STEVE itself. It follows Autodesk's documented `Scripts.itemsByName`, `Script.stop()`, `Script.isRunning`, `Script.run()`, and `Script.folder` lifecycle: locate exactly one valid STEVE program whose reported folder matches the managed installation, stop it, replace only the managed folder with rollback, purge only STEVE Python modules, run the program again, and verify that it is running. Requests are revalidated when consumed, including version syntax, staged-package naming, manifest version, and trusted path. A failed handoff restores the old folder and attempts to run it. The transaction journal records the backup path before the rename and performs conservative startup recovery; ambiguous states retain the journal and require manual recovery rather than deleting an existing installation. The identity check rejects symlinked path components, including dangling and parent links, immediately before lifecycle operations. This is not a race-resistant defense against an adversarial local process changing the AddIns tree concurrently; that threat is outside the current updater threat model. The existing atomic staging, checksum validation, rollback, and quit-Fusion fallback remain the recovery boundary. This path is intentionally not enabled until the helper has been installed by the migration release and the complete sequence has passed on a real Windows and macOS Fusion installation; Linux tests use API stand-ins only. Fusion lifecycle calls are synchronous main-thread operations, so this code cannot safely impose an out-of-thread timeout if `stop()` or `run()` hangs. That behavior, callback timing, file-lock release, and module reload semantics must be validated in real Fusion before enabling the feature. If the helper cannot prove a safe stop or restart, it must leave the files untouched or restore the backup and instruct the user to restart Fusion.

## Trusted local filesystem boundary

The in-Fusion updater validates canonical paths, symlink safety, complete file membership, and SHA-256 inventories repeatedly before lifecycle and recovery operations. These checks are fail-closed evidence for the normal trusted-user filesystem model, but they are not atomic against a concurrent same-user process that mutates staged, installed, backup, request, journal, receipt, or acknowledgement paths between validation and subsequent open, copy, rename, delete, or execution. Protecting against hostile concurrent local mutation requires stronger descriptor-relative operations, filesystem locks, or platform-specific atomic primitives and is outside the current implementation boundary. Windows remains on the detached installer path because equivalent durability is not implemented.
